...
(define (write-new-manifest path name-parts lib? dialect)
  (call-with-output-file path
    (lambda (out)
      ...
      (display (if lib?
                   "#f"
                   (if (eq? dialect 'r6rs) "\"main.sps\"" "\"main.scm\""))
               out)
      (display ")" out)
      (newline out)
      (display "  (tests " out)
      (display (if (eq? dialect 'r6rs) "\"tests/main.sps\"" "\"tests/main.scm\"") out)
      (display ")" out)
      (newline out)
      (display "  (benches " out)
      (display (if (eq? dialect 'r6rs) "\"benches/main.sps\"" "\"benches/main.scm\"") out)
      (display ")" out)
      (newline out)
      (display "  (examples " out)
      (display (if (eq? dialect 'r6rs) "\"examples/main.sps\"" "\"examples/main.scm\"") out)
      (display "))" out)
      (newline out)
      (newline out)
      (if lib?
          ...
          (begin
            ...
            (if (eq? dialect 'r6rs)
                (display "(rnrs)" out)
                (display "(scheme base) (scheme write)" out))
            (display "))" out)
            (newline out)
            (newline out)
            (display "(dev-dependencies)" out)))
...
(define (write-new-library path name-parts dialect)
  (call-with-output-file path
    (lambda (out)
      (if (eq? dialect 'r6rs)
          (begin
...
(define (write-new-main path name-parts dialect)
  (call-with-output-file path
    (lambda (out)
      (if (eq? dialect 'r6rs)
          (begin
	...
	(define (write-new-test path name-parts dialect)
	  (call-with-output-file path
	    (lambda (out)
	      (if (eq? dialect 'r6rs)
	          (begin
	...
	(define (write-new-bench path name-parts dialect)
	  (call-with-output-file path
	    (lambda (out)
	      (if (eq? dialect 'r6rs)
	          (begin
	...
	(define (write-new-example path name-parts dialect)
	  (call-with-output-file path
	    (lambda (out)
	      (if (eq? dialect 'r6rs)
	          (begin
...
(define (write-new-gitignore path)
  (call-with-output-file path
	...
	(define (starter-dialect-value->symbol value)
	  (cond
	   ((not value) #f)
	   ((string=? value "r7rs") 'r7rs)
	   ((string=? value "r6rs") 'r6rs)
	   (else (usage-error "unknown starter dialect" value))))

	(define (starter-scheme-default-dialect scheme)
	  (cond
	   ((implementation-mode-for-dialects scheme '(r7rs)) 'r7rs)
	   ((implementation-mode-for-dialects scheme '(r6rs)) 'r6rs)
	   (else (usage-error "selected scheme cannot run r7rs or r6rs starters" scheme))))

	(define (starter-selected-dialect cmd)
	  (let* ((scheme (command-selected-scheme cmd))
	         (explicit-scheme? (command-option cmd "scheme" #f))
	         (raw-dialect (command-option cmd "dialect" #f))
	         (dialect (or (starter-dialect-value->symbol raw-dialect)
	                      (starter-scheme-default-dialect scheme))))
	    (when (and (or explicit-scheme? (not raw-dialect))
	               (not (implementation-mode-for-dialects scheme (list dialect))))
	      (usage-error "selected scheme does not support starter dialect"
	                   (list scheme dialect)))
	    dialect))

	(define (starter-spec dir raw-name command lib? dialect)
	  (let ((name-parts (starter-name-parts raw-name)))
	    ...
	    (let* ((src-dir (path-join dir "src"))
	           ...
	           (library-path (path-join library-dir
	                                    (string-append library-leaf
	                                                   (if (eq? dialect 'r6rs) ".sls" ".sld"))))
	           (manifest-path (path-join dir "kons.scm"))
	           (gitignore-path (and (string=? command "new")
	                                (path-join dir ".gitignore")))
	           (main-path (path-join src-dir
	                                 (if (eq? dialect 'r6rs) "main.sps" "main.scm")))
	           (test-dir (path-join dir "tests"))
	           (program-extension (if (eq? dialect 'r6rs) ".sps" ".scm"))
	           (test-path (path-join test-dir (string-append "main" program-extension)))
	           (bench-dir (path-join dir "benches"))
	           (bench-path (path-join bench-dir (string-append "main" program-extension)))
	           (example-dir (path-join dir "examples"))
	           (example-path (path-join example-dir (string-append "main" program-extension))))
	      `((directory . ,dir)
	        (name . ,name-parts)
	...
	(define (starter-plan command spec)
	  `(,(string->symbol (string-append command "-plan"))
	    ...
	    (dialect ,(starter-spec-ref spec 'dialect))
	...
	(define (write-starter-package spec)
	  (let ((name-parts (starter-spec-ref spec 'name))
	    ...
	    (write-new-library (starter-spec-ref spec 'library-path) name-parts dialect)
	    (unless (eq? (starter-spec-ref spec 'kind) 'lib)
	      (write-new-main (starter-spec-ref spec 'main-path) name-parts dialect))
	    (write-new-test (starter-spec-ref spec 'test-path) name-parts dialect)
	    (write-new-bench (starter-spec-ref spec 'bench-path) name-parts dialect)
	    (write-new-example (starter-spec-ref spec 'example-path) name-parts dialect)))
