(define-library (kons registry)
  ...
  (begin
    ...
    (define (sexp-name->json value)
      (cond
        ((pair? value) (list->vector (map sexp-string-value value)))
        (else (sexp-string-value value))))
    ...
    (define (sexp-dependency->json form)
      (let ((fields (if (and (pair? form) (eq? (car form) 'dependency)) (cdr form) '())))
        `((type . ,(sexp-symbol-string (sexp-field fields 'type 'registry)))
          (name . ,(sexp-name->json (sexp-field fields 'name "")))
          ...
          (source . ,(let ((value (sexp-field fields 'source #f)))
                       (and value (sexp-string-value value))))
    ...
    (define (registry-json-source-type dep)
      (registry-json-symbol
        (json-ref dep 'type (json-ref dep 'sourceType "registry"))
        'registry))

    (define (registry-json-external-name value)
      (cond
        ((vector? value) (registry-json-external-name (vector->list value)))
        ((list? value)
          (map (lambda (item)
                (cond
                  ((symbol? item) item)
                  ((number? item) item)
                  ((and (string? item) (non-empty-string? item)) (string->symbol item))
                  (else 'unknown)))
            value))
        ((symbol? value) (symbol->string value))
        ((string? value) value)
    ...
    (define (registry-json-external-type? type)
      (memq type '(akku snow)))
    ...
    (define (registry-json-dependency registry dep)
      (let ((type (registry-json-source-type dep))
            (features (registry-json-symbol-list (json-ref dep 'features '#())))
        ...
        `((type . ,type)
          (name . ,(if (registry-json-external-type? type)
                     (registry-json-external-name (json-ref dep 'name ""))
                     (registry-json-name (json-string-ref dep 'name ""))))
          ...
          ,@(if (registry-json-external-type? type)
              `((source . ,(let ((source (json-string-ref dep 'source "")))
                            (if (string=? source "")
                              (let ((dep-registry (json-string-ref dep 'registry "")))
                                (if (string=? dep-registry "")
                                  (if (eq? type 'snow) "snow" "akku")
                                  dep-registry))
                              source))))
              '())
          (scope . ,(registry-json-symbol (json-ref dep 'kind "runtime") 'runtime))
    ...
    (define (sparse-index-dependency registry dep)
      `((type . ,(json-ref dep 'type "registry"))
        (name . ,(json-ref dep 'name ""))
        (req . ,(json-string-ref dep 'req "*"))
        (kind . ,(json-ref dep 'kind "runtime"))
        (registry . ,(let ((dep-registry (json-string-ref dep 'registry "")))
                      (if (string=? dep-registry "") registry dep-registry)))
        (source . ,(json-ref dep 'source #f))
